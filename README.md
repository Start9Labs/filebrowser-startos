...
This package names two ways forward. **NextExplorer** is the recommended replacement: a separate package on the Start9 registry whose Import Files from File Browser action reflinks this package's `data` volume into a drive of its own, files only. **FileBrowser Quantum** is the in-place one: a maintained fork of this project, published under this same package id as the `#quantum` flavor, so it reaches users as a flavor switch on this listing. Switching to Quantum preserves files, users, and passwords.
