...
This repo packages [File Browser](https://github.com/filebrowser/filebrowser) for StartOS.
...
File Browser is pulled in as the upstream `filebrowser/filebrowser` Docker Hub image. To track a new upstream release:

1. Bump `dockerTag` in `startos/manifest/index.ts` to `filebrowser/filebrowser:v<new version>`.
