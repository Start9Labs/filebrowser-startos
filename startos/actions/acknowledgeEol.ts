...
export const acknowledgeEol = sdk.Action.withInput(
  ...
  async ({ effects }) => ({
    ...
    description: '',
    warning: i18n(
      '<b>File Browser is no longer maintained.</b> Its developers have wound the project down: 2.63.23 is the final release and the source repository is being archived. There will be no further releases and no security fixes.<br><br>Nothing changes today. Your files are untouched, the service keeps working, and you may keep using it — but it will not be repaired if a problem is found in it, so plan to move.<br><br><b>NextExplorer is the recommended replacement.</b> It is a different file manager, on the Start9 Registry, and its <b>Import Files from File Browser</b> action copies your files into it in seconds without touching this service. Files only: you re-create accounts, folder permissions and share links there.<br><br><b>FileBrowser Quantum is the other route.</b> It is a maintained fork of this same project, offered under this listing as a separate flavor, so switching keeps your files, your users, and their passwords.<br><br><b>Before you switch to Quantum:</b><ul><li><b>Back up this service.</b> The switch is permanent — File Browser will not take your data back, and restoring that backup is the only way to return.</li><li><b>Re-check restricted accounts.</b> Per-user folder restrictions do not carry over, so an account confined to one folder will see everything.</li><li><b>Re-create your share links.</b> Existing ones stop working.</li></ul>',
    ),
    allowedStatuses: 'any',
    group: null,
    visibility: 'hidden',
